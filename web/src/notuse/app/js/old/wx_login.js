$(function(){
	...
	function init(){
		...
		if(hash==="#timeout"){
			$errorBox.html("登录超时，请重新登录!").show();
		}
		
		try{
			var user=window.localStorage.rememberUser;
			if(user){//如果localstorage里有用户，直接替换
				$username.val(user);
			}
		}catch(ex){
					...
					case "0":
						...
						if (urlHref.indexOf("?url=")!=-1) {//带参数的URL跳转
							...
							if(urlHref[1]===""){
								...
								var urlHref2=decodeURIComponent(urlHref[1]);
								if(/share\/index/ig.test(urlHref2)){
									window.location.href =urlHref2+"#random="+new Date().getTime();
								}
								else{
									window.location.href =urlHref2;
								}
							}
						} else {//默认跳回首页
							window.location.href = defaultPath;
